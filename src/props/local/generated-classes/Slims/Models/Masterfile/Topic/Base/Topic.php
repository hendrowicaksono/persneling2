...
/**
 * Base class that represents a row from the 'mst_topic' table.
 *
 *
 *
 * @package    propel.generator.Slims.Models.Masterfile.Topic.Base
 */
abstract class Topic implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Slims\\Models\\Masterfile\\Topic\\Map\\TopicTableMap';
    ...
    /**
     * Initializes internal state of Slims\Models\Masterfile\Topic\Base\Topic object.
     */
     ...
     * @return $this|\Slims\Models\Masterfile\Topic\Topic The current object (for fluent API support)
     */
    public function setTopicId($v)
    {
        ...
        if ($this->topic_id !== $v) {
            $this->topic_id = $v;
            $this->modifiedColumns[TopicTableMap::COL_TOPIC_ID] = true;
        }
    ...
    public function setTopic($v)
    {
        ...
        if ($this->topic !== $v) {
            $this->topic = $v;
            $this->modifiedColumns[TopicTableMap::COL_TOPIC] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TopicTableMap::translateFieldName('TopicId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->topic_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TopicTableMap::translateFieldName('Topic', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = TopicTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Slims\\Models\\Masterfile\\Topic\\Topic'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC_ID)) {
            $modifiedColumns[':p' . $index++]  = 'topic_id';
        }
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC)) {
            $modifiedColumns[':p' . $index++]  = 'topic';
        }

        $sql = sprintf(
            'INSERT INTO mst_topic (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Slims\Models\Masterfile\Topic\Topic
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Slims\Models\Masterfile\Topic\Topic The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC_ID)) {
            $criteria->add(TopicTableMap::COL_TOPIC_ID, $this->topic_id);
        }
        if ($this->isColumnModified(TopicTableMap::COL_TOPIC)) {
            $criteria->add(TopicTableMap::COL_TOPIC, $this->topic);
        }
    ...
    /**
     * Generic method to set the primary key (topic_id column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Slims\Models\Masterfile\Topic\Topic (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setTopicId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Slims\Models\Masterfile\Topic\Topic Clone of current object.
     * @throws PropelException
     */
    ...
    public function setBiblioTopics(Collection $biblioTopics, ConnectionInterface $con = null)
    {
        ...
        foreach ($biblioTopicsToDelete as $biblioTopicRemoved) {
            $biblioTopicRemoved->setTopic(null);
        }
     ...
     * @return ObjectCollection|BiblioTopic[] List of BiblioTopic objects
     */
    public function getBiblioTopicsJoinBiblio(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = BiblioTopicQuery::create(null, $criteria);
    ...
    protected function doAddBiblio(Biblio $biblio)
    {
        ...
        if (!$biblio->isTopicsLoaded()) {
            $biblio->initTopics();
            $biblio->getTopics()->push($this);
        } elseif (!$biblio->getTopics()->contains($this)) {
            $biblio->getTopics()->push($this);
        }
    ...
    public function removeBiblio(Biblio $biblio)
    {
        if ($this->getBiblios()->contains($biblio)) {
            ...
            if ($biblio->isTopicsLoaded()) {
                ...
                $biblio->getTopics()->removeObject($this);
            }
